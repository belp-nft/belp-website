...
export function useWallet(onConnected?: (info: Connected) => void) {
  ...
  const refreshSolBalance = useCallback(async (address?: string) => {
    const targetAddress = address || solAddress;
    ...
    try {
      ...
      console.log("✅ Balance loaded from web3:", lamports, "address:", targetAddress);
    } catch (error: any) {
  ...
  const authenticateWallet = useCallback(
    async (
      ...
      if (existingToken && AuthService.isTokenValid()) {
        ...
        await refreshSolBalance(walletAddress);
    ...
    const handleWalletConnected = async () => {
      ...
      for (const [walletType, config] of Object.entries(WALLET_CONFIGS)) {
        ...
        if (provider?.isConnected === true && provider?.publicKey) {
          ...
          // Load balance after syncing state
          await refreshSolBalance(addr);
